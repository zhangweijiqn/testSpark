...
object ItemBased {
  ...
  def testCalcSim(): Unit ={
    ...
    /*对上面代码进行优化，返回类型不同，下面的代码都需要修改一下
    val numRatersPerItem = ratings.map(x=>(x._1,(1,x._3,math.pow(x._3,2)))).reduceByKey((a,b)=>(a._1+b._1,a._2+b._2,a._3+b._3)).map(x=>(x._1,x._2._1,x._2._2,x._2._3))
    */
    //将上面得到的结果再放回原记录中
    ...
    val similarities = tempSimilarities.groupByKey().flatMap(x => { //(Long,Iterable(Long,Double))
      x._2.map(temp => (x._1, (temp._1, temp._2))).toList.sortWith((a, b) => a._2._2 > b._2._2).take(50)    //可以使用List top优化（隐式转换）
    })
    ...
    /*  商品推荐  */
    // ratingsInverse format (item,(user,raing))
    val ratingsInverse = ratings.map(r => (r._2, (r._1, r._3)))
